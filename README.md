...
- `ulimit -c unlimited` (с предупреждением, если лимит не поднялся —
  ...
  делает `LD_PRELOAD`; на старых glibc (Debian 11, CentOS) переменная
  действует без preload. С включённой проверкой heap-layout отличается
  от боевого запуска — краши могут проявляться в других точках
