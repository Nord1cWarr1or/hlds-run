...
- переменные окружения движка — по явному списку (`LD_LIBRARY_PATH`,
  `MALLOC_CHECK_`, `MALLOC_PERTURB_`, `TERM`, `HOME`); `STEAM_*` и `PATH`
  в отчёт не попадают. Файл отчёта создаётся с правами `600`
...
- `ulimit -c unlimited` (с предупреждением, если лимит не поднялся —
  ...
  это layout-зависимость, и это само по себе зацепка
...
200 МБ через `--output-limit` страхует от спамящего консоль), откуда
