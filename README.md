...
- `ulimit -c unlimited` (с предупреждением, если лимит не поднялся —
  контейнер/сервисные ограничения) и `MALLOC_CHECK_=3` — жёсткая проверка
  кучи glibc: порча heap приводит к громкому `abort` вместо тихих
  последствий. **Важно:** на glibc ≥ 2.34 (Debian 12+, Arch, Ubuntu 22+)
  проверка работает только при предзагрузке `libc_malloc_debug.so` —
  скрипт сам находит её через `ldconfig` (32-битную, под бинарник) и
  передаёт `LD_PRELOAD` **только серверному процессу** (через launcher/
  `env`; хелперы самой обёртки preload не видят — нет шума
  «wrong ELF class» от 64-битных утилит); на старых glibc (Debian 11,
  CentOS) переменная действует без preload. **Краши под `-debug` могут
  отличаться от боевых:** отладочный malloc (и preload, и нативная
  проверка) меняет раскладку кучи — layout-чувствительная порча
  срабатывает в других местах, а в худшем случае краш исчезает совсем
  (проверено на Debian 13: краши в `SteamGameServer_Init` пропадали
  под preload). Если краш воспроизводится в проде, но не под `-debug` —
  это layout-зависимость, и это само по себе зацепка. **Свой
  `LD_PRELOAD` оператора** (например, из systemd-юнита) не вырезается:
  он снимается с окружения обёртки (чтобы 64-битные хелперы не шумели),
  но передаётся серверу вместе с heap-debug библиотекой; в лог пишется
...
- `MALLOC_PERTURB_=170` (в скрипте закомментировано, включается вручную):
  ...
  добавляет ощутимые накладные расходы на каждый malloc/free. Работает
  на всех версиях glibc, preload не требует
...
- **Активная диагностика кучи в `-debug`**: `ulimit -c unlimited`
  (в оригинале — 2000 блоков) и `MALLOC_CHECK_=3` — heap-порча даёт
  громкий abort
