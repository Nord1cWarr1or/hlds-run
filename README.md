...
- `ulimit -c unlimited` (с предупреждением, если лимит не поднялся —
  контейнер/сервисные ограничения). Больше **никаких изменений
  окружения**: `MALLOC_CHECK_`, `MALLOC_PERTURB_`, `LD_PRELOAD` с
  отладочным malloc скрипт сознательно не трогает. Причина — отладочный
  аллокатор glibc (`libc_malloc_debug.so`, glibc ≥ 2.34) меняет
  раскладку кучи: layout-чувствительная порча перестаёт срабатывать и
  краш исчезает под `-debug` (проверено на Debian 13: краши в
  `SteamGameServer_Init` пропадали при preload). Воспроизводимость важнее
  громкости: краш под `-debug` падает точно так же, как в проде, и дамп
  снимается при каждом запуске
- если нужен активный радар кучи — переменные выставляются вручную
  в окружении перед запуском: `LD_PRELOAD=/lib/i386-linux-gnu/libc_malloc_debug.so.0`
  (на glibc ≥ 2.34; путь 32-битный под сервер), `MALLOC_CHECK_=3`,
  `MALLOC_PERTURB_=170` (заполняет новые аллокации байтом 0xAA,
  освобождённые — `~0xAA`; ловит use-after-free, даёт ощутимые тормоза)
...
- **`ulimit -c unlimited` в `-debug`** (в оригинале — 2000 блоков);
  окружение сервера при этом не меняется — краши воспроизводимы с продом
